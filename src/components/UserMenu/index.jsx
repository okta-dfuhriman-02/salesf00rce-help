...
const UserMenu = () => {
	const {
		...
		profile: { id, picture, email, nickName, firstName, displayName },
	} = Auth.useAuthState();

	return (
		<>
			{isPendingLogout || isPendingUserInfoFetch || !id ? (
				<div
								...
								<LDS.Button variant='base' className='tds-avatar'>
									...
									>
										<div
											...
											style={{ color: 'black' }}
										>
											{nickName ?? firstName ?? displayName}
										</div>
										<LDS.Avatar
											imgSrc={
												isUrl(picture)
